...
class TechnicalIndicators:
    ...
    def generate_signal(bars: List[HistoricalBar],
        ...
        if use_bb:
            ...
            prev_upper = bb_result.upper[-2]
            prev_lower = bb_result.lower[-2]
            ...
            # LONG: Precio rebota desde la banda inferior (estaba debajo, ahora sube)
            if prev_price <= prev_lower and current_price > lower:
                signals.append('LONG')
                confidences.append(0.75)
                reasons.append('BB: Rebote desde banda inferior')
            # LONG: Precio está muy cerca de la banda inferior (sobreventa)
            elif current_price <= lower * 1.002:  # 0.2% de tolerancia
                signals.append('LONG')
                confidences.append(0.70)
                reasons.append('BB: Precio en zona de sobreventa')
            # SHORT: Precio rebota desde la banda superior (estaba arriba, ahora baja)
            elif prev_price >= prev_upper and current_price < upper:
                signals.append('SHORT')
                confidences.append(0.75)
                reasons.append('BB: Rebote desde banda superior')
            # SHORT: Precio está muy cerca de la banda superior (sobrecompra)
            elif current_price >= upper * 0.998:  # 0.2% de tolerancia
                signals.append('SHORT')
                confidences.append(0.70)
                reasons.append('BB: Precio en zona de sobrecompra')
        ...
        if use_stoch_rsi:
            ...
            indicators_data['stoch_rsi'] = {
                ...
                'oversold': float(stoch_rsi_oversold),
                'overbought': float(stoch_rsi_overbought)
            }

            # LONG: Cruce alcista (K cruza D hacia arriba) en zona de sobreventa
            if prev_k <= prev_d and k_value > d_value and prev_k < stoch_rsi_oversold:
                signals.append('LONG')
                confidences.append(min(0.90, 0.75 + (stoch_rsi_oversold - prev_k) / stoch_rsi_oversold * 0.15))
                reasons.append(f'StochRSI cruce alcista desde sobreventa (K={k_value:.1f} < {stoch_rsi_oversold})')
            # SHORT: Cruce bajista (K cruza D hacia abajo) en zona de sobrecompra
            elif prev_k >= prev_d and k_value < d_value and prev_k > stoch_rsi_overbought:
                signals.append('SHORT')
                confidences.append(min(0.90, 0.75 + (prev_k - stoch_rsi_overbought) / (100 - stoch_rsi_overbought) * 0.15))
                reasons.append(f'StochRSI cruce bajista desde sobrecompra (K={k_value:.1f})')
        ...
        # Contar indicadores habilitados
        enabled_indicators_count = sum([
            1 for use in [use_smi, use_macd, use_bb, use_ma, use_stoch_rsi, use_vwap, use_supertrend, use_kdj]
            if use
        ])
        ...
        # LÓGICA DE CONSENSO: TODOS los indicadores deben coincidir
        # Si seleccionas 2 indicadores, ambos deben dar la misma señal
        if enabled_indicators_count > 1:
            # Consenso total requerido (100%)
            if long_count == enabled_indicators_count and short_count == 0:
                # TODOS los indicadores dan LONG
                final_signal = 'LONG'
                final_confidence = np.mean(confidences)
                final_reason = f'✓ CONSENSO TOTAL ({long_count}/{enabled_indicators_count}): ' + ' + '.join(reasons)
            elif short_count == enabled_indicators_count and long_count == 0:
                # TODOS los indicadores dan SHORT
                final_signal = 'SHORT'
                final_confidence = np.mean(confidences)
                final_reason = f'✓ CONSENSO TOTAL ({short_count}/{enabled_indicators_count}): ' + ' + '.join(reasons)
            else:
                # Desacuerdo entre indicadores -> NO OPERA
                final_signal = 'NEUTRAL'
                final_confidence = 0.0
                final_reason = f'❌ SIN CONSENSO: {long_count}L/{short_count}S (requiere {enabled_indicators_count}/{enabled_indicators_count})'
        else:
            # Un solo indicador: usar lógica de mayoría simple
        ...
        return {
            ...
            'votes': {
                ...
                'total_enabled': enabled_indicators_count,
                'consensus_required': enabled_indicators_count > 1
            }
        }
